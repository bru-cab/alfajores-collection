...
def serve_image(filename):
    ...
    # Some deployments store only the lightweight JPEG companion files.
    if filename.endswith('.png') and filename.startswith('page_'):
        page_stem = Path(filename).stem
        jpg_fallback = os.path.join(app.config['IMAGES_FOLDER'], f"{page_stem}_1.jpg")
        if os.path.exists(jpg_fallback):
            return send_file(jpg_fallback)
